...
class Lemonway extends PaymentModule
{
    ...
    public function __construct()
    {
        ...
        $this->confirmUninstall = $this->l('Are you sure you want to uninstall? You will lose your data!');
    ...
    public function getCustomerCard($id_customer)
    {
        if (is_null($this->current_card)) {
            $query = 'SELECT * FROM `' . _DB_PREFIX_ . 'lemonway_oneclic` lo WHERE lo.`id_customer` = ' . (int) pSQL($id_customer);
    ...
    public function saveWkToken($id_cart)
    {
        ...
        $query = 'UPDATE `' . _DB_PREFIX_ . 'lemonway_wktoken` SET `wktoken` = \'' . pSQL($wkToken) . "' WHERE `id_cart` = " . (int) pSQL($id_cart);

        // If cart haven't wkToken we insert it
        if (!$this->checkIfCartHasWkToken($id_cart)) {
            $query = 'INSERT INTO `' . _DB_PREFIX_ . 'lemonway_wktoken` (`id_cart`, `wktoken`) VALUES (\'' . (int) pSQL($id_cart) . '\',\'' . pSQL($wkToken) . '\') ';
        }
    ...
    public function getCartIdFromToken($wktoken)
    {
        if ($id_cart = Db::getInstance()->getValue(
            'SELECT `id_cart` FROM `' . _DB_PREFIX_ . 'lemonway_wktoken` lw WHERE lw.`wktoken` = \'' . pSQL($wktoken) . "'"
        )) {
            return $id_cart;
        }
    ...
    public function ajaxPlaceOrder()
    {
        ...
        $comment = Configuration::get('PS_SHOP_NAME') . " - " . $cart->id . " - " . $customer->lastname . " " . $customer->firstname . " - " . $customer->email;
        ...
        if (array_key_exists($this->context->language->iso_code, $this->supportedLangs)) {
            $language = $this->supportedLangs[$this->context->language->iso_code];
        }
